---
notion-id: 2fe57008-e885-8198-ad39-e0a2a491a675
notion-url: "https://www.notion.so/Media-Toggles-Embeds-2fe57008e8858198ad39e0a2a491a675"
notion-frozen-at: "2026-02-10T15:44:56Z"
notion-last-edited: "2026-02-05T19:44:00.000Z"
notion-database-id: 2fe57008-e885-8003-b1f3-cc05981dc6b0
Phone: +1-555-0199
Approved: false
Tags:
  - urgent
  - bug
Created: "2026-02-05T19:44:00.000Z"
"Contact Email": null
Attachments: []
Description: Tests images, bookmarks, embeds, toggles, and dividers
"Last Edited": "2026-02-05T19:44:00.000Z"
"Due Date": null
Score: 0
Category: Marketing
Website: null
Assignee: []
Related: []
---
## Images
![A beautiful landscape photo](https://images.unsplash.com/photo-1506744038136-46273834b3fb?w=800)
![LaTeX example rendering](https://upload.wikimedia.org/wikipedia/commons/thumb/0/09/Latex_example.png/320px-Latex_example.png)
## Bookmarks
[Notion API Documentation](https://developers.notion.com/)
[GitHub - Go Programming Language](https://github.com/golang/go)
## Toggles
> [!note]+ Click to expand: Basic Toggle
> 
> This content is inside a toggle block.
> It can contain **formatted text** and multiple paragraphs.
> [!note]+ Toggle with nested content
> 
> First paragraph inside toggle.
> - Bullet inside toggle
> - Another bullet
> 1. Numbered item inside toggle
> 2. Second numbered item
> [!note]+ Empty-ish toggle
> 
> Just a single line of content.
---
## Divider Above
Content between dividers.
---
## More Content After Divider
Final paragraph with some $\alpha + \beta = \gamma$ inline math.